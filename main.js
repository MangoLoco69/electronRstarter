...
const port = "6969" //9191 
const child = require('child_process');
const MACOS = "darwin"
const WINDOWS = "win32" //it is really win 64 tho 
...
if(process.platform == WINDOWS){
  ...
  execPath = path.join(app.getAppPath(), "R-Portable-Win", "bin", "RScript.exe" ) // This points to the R engine essentially.
...
function createWindow () {
  ...
  console.log('create-window')
    ...
    loading.once('show', () => {
      ...
      mainWindow = new BrowserWindow({webPreferences:{nodeIntegration:false}, show:false, width: 1080, height: 720, title:""})
      /*
      ...
      */
      ...
      // This is where we load the HTML by referrring to the Shiny App :
...
function cleanUpApplication(){
  ...
  // I changed the cleanUpApplication to do a /shutdown call in addition to app.quit
  if (childProcess) {
    // Send a request to the Shiny server to shut it down
    fetch('http://127.0.0.1:' + port + '/shutdown')
      .then(() => {
        childProcess.kill(); // Terminate the Electron child process
        //childProcess = null;
        if (killStr !== "") {
          child.execSync(killStr); // Execute additional cleanup if needed
        }
        app.quit(); // Quit the Electron app
        console.log('EVENT::app shut down')
      })
      .catch((error) => {
        console.error('Error shutting down the server:', error);
        app.quit(); // Quit the Electron app even if there was an error
      });
  } else {
    app.quit(); // Quit the Electron app if childProcess doesn't exist
  }
